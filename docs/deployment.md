...
- **Source/license UI:** Every running web UI must offer a visible Source & Trust entry point. The current app shows it on the public login surface and Account view, with Source Code, AGPL License, Build Details, and Locale Data links backed by `/source-info`, `/.well-known/keystone-build.json`, and `/.well-known/keystone-locales.json`. If public account flows use Cloudflare Turnstile or another third-party security check, this surface must disclose that dependency in plain language.
